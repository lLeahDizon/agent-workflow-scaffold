...
- 文本文件通过 `agent-workflow-scaffold` 托管区块（managed block）更新，避免覆盖用户手写内容。
- JSON 文件使用结构化合并。
- 生成路径必须位于目标项目根目录内。
...
- Trae：生成 `.trae/agents/*.md`，并保留 `.trae/skills/<project-id>-workflow/references/subagents.md` 作为说明和兜底上下文。
...
CLI 会扫描本地或全局 `SKILL.md` 元信息，并把推荐结果写入项目本地工作流 skill 的 `references/skills.md`。
...
脚手架只生成项目工作流 skill，不会默认复制、安装或修改用户全局 skill。

## 协作手册（Workflow Playbook）

脚手架会在项目本地工作流 skill 中生成中文 AI Coding 协作手册：

```text
references/workflow-playbook.md
```

内容包括任务定义、计划分析、小步实现、自检验证、人工评审（Review）、Git/PR 交接、worktree 隔离和复盘沉淀流程。

## 内部发布

当前包配置为发布到内部 npm 私有源（registry）：
...
`prepack` 会在打包前执行 TypeScript 构建，`prepublishOnly` 会在发布前执行构建、测试和打包 dry-run 预览。
