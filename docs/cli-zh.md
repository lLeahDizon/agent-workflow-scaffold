...
当前版本：`0.0.18`。版本变更记录见项目根目录 `CHANGELOG.md`。
...
- 命令行中提前传入的 `--root`、`--target`、`--project-type`、`--agent-provider`、`--loop-engineering`、`--write` 会作为向导默认值，但仍可在问答中调整。
...
## 15. workflow-playbook：中文 AI Coding 协作流程
...
## 16. loop-engineering：可选循环工程参考配置
